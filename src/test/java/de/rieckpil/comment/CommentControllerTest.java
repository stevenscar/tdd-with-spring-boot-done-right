...
class CommentControllerTest {
  ...
  void shouldAllowAnonymousUsersToGetAllComments() throws Exception {
    ...
    when(commentService.findAll()).thenReturn(
      List.of(
        ...
        new Comment(UUID.randomUUID(), "2", "Hello World C", LocalDate.now().minusDays(2))
      )
    );
  ...
  void shouldCreateCommentWhenUserIsAuthenticatedAndAdmin() throws Exception {
    ...
    when(commentService.createComment(any(), anyString()))
      .thenReturn(uuid);
    ...
    mockMvc
      ...
      .andExpect(header().string("Location", Matchers.containsString("/api/comments/%s".formatted(uuid))))
      .andDo(print())
      .andReturn();
  }
